#!/usr/bin/env python3
"""Deterministic evidence pre-check for mathematical-modeling claim audits.

Before an author or reviewer judges whether a conclusion is supported,
mechanically verify that its cited evidence exists and that the cited
number/string can be located in the local source file. This is deliberately a
fail-closed existence check: a `verified` result never means the model is
correct, the statistic is valid, or the claim may enter the paper.

The numeric matcher uses an allow-list of safe token boundaries. Ambiguous
dates, versions, fractions, locale groupings and Unicode-adjacent numerals are
not promoted to `verified`; they require direct inspection and, where needed,
...
def main() -> int:
    ap = argparse.ArgumentParser(description="Deterministic mathematical-modeling evidence pre-check.")
