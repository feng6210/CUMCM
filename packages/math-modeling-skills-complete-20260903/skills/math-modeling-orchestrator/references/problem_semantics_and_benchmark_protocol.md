...
以下触发条件仍受本次用户授权和适用赛事规则约束。“仅本地/不查答案”优先于可公开访问，不因 training 模式或强声明自动联网。只要求学习论文风格时，不自动运行外部数值方案。缺少允许范围内的挑战证据，应缩小声明或报告缺口，不能自行扩大检索、计算预算或改变模型。
...
- 外部可行解在同 evaluator 下领先超过预设容差：当前“最佳/稳定最优”声明立即 `FAIL` 或降为 `PARTIAL`；只有仍在已批准计算范围内才能继续求解，超出预算/模型/阈值时先重新提案获批。
