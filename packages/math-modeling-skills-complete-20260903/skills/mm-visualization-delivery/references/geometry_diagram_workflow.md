...
示意图属于 `mechanism`，目标是让读者从本题对象和相互关系直接读到方程中的量。用户已确认并更新为 [TikZ黑白线稿与剖面线样式](approved_tikz_schematics.md) 的资产是默认可运行起点；定日镜、刚杆、传热材料都只是示例，不应固化为专用题型。
当前示意图默认纯黑线条与文字、白色背景，无彩色/灰色区域填充；材料、部件和方向用虚实线、线宽、箭头、剖面线和直接标签区分。历史彩色样图可借构图而不继承颜色，结果图的 `cumcm-vivid` 默认不受此规则影响。
...
这三类图可以互补，但不能互相冒充，也不要求每个模型都画齐：

- **几何/运动图**说明实体、坐标、自由度、连接与运动学关系；没有画某个力不表示该力不存在。
- **完整隔离受力图**先明确所隔离的系统边界，再列该边界上的外力、力矩及必要的约束反力。相互作用只在隔离体外部时列入，避免整体图重复画内部作用；省略项须有平衡、对称或理想化依据。
- **消约束后的广义作用图**解释势能、耗散或广义力如何进入已选坐标方程；注明哪些反力已由相容运动学/虚功消去。消去不等于反力为零，也不能将这种图称为完整孤立体受力图。广义力不是必有唯一空间作用点的力箭头；必要时用部件关系和带符号公式说明传递。

图注和紧邻公式应使用同一推导阶段。区分原始力、静平衡预载以及平衡消项后的增量力：若图写原始弹性力而方程使用增量势能，明确哪一步与重力或其他恒定项合并，不能用一句“已消去”暗示图上的原始公式已经改变。恢复项只能按模型计入一次，不为补图另添一套。

标注位置与有效锚点分开：为避免重叠可移开弹簧/阻尼器符号或标签，但不能默默移动其有效连接点、力臂或参考轴；元件作移位示意时标明真正参考点，存在实际偏置则沿用已批准几何关系。力矩弧先对照广义坐标正方向，再核对两部件间作用/反作用的符号；“等大反向”按所用坐标与参考轴说明，不能只凭两支屏幕箭头判断。

同一实体跨几何、作用和结果图保持符号、参考点、正方向和绝对/相对量定义一致；示意图以一致线型/标签对应实体，结果图内部再保持颜色角色，不要求跨两类图复用颜色。视角或参考轴改变时显式说明变换。迁移的是这些检查，不是某一题的实体、数字、图数或画面。
...
默认 `style.color_mode=monochrome`，包括带旧彩色字段的 spec 也输出黑线黑字白底；区域/多边形可用 `hatch: diagonal|cross`。仅用户明确另选彩色或复现历史样式时设置 `color_mode=color`；这不是数据结果图渲染器，不能将结果数据传入此回退以改变其配色。
