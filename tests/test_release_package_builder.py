...
class ReleasePackageBuilderTests(unittest.TestCase):
    def test_preview_zip_is_deterministic_and_contains_fresh_metadata(self):
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual(report_a["skill_count"], 23)
            ...
            with zipfile.ZipFile(zip_a, "r") as archive:
                ...
                validation_bytes = archive.read(f"{prefix}/VALIDATION_REPORT.json")
                validation = json.loads(validation_bytes.decode("utf-8"))
                self.assertEqual(manifest["schema_version"], "3.0")
                self.assertEqual(manifest["skill_count"], 23)
                self.assertIn("VALIDATION_REPORT.json", manifest["regenerated_metadata"])
                validation_entry = next(item for item in manifest["files"] if item["path"] == "VALIDATION_REPORT.json")
                self.assertTrue(validation_entry["generated"])
                self.assertEqual(validation_entry["sha256"], builder.sha256_bytes(validation_bytes))
                self.assertIn(validation_entry["sha256"] + "  VALIDATION_REPORT.json", sums)
                ...
                self.assertIn("skills/mm-paper-humanizer/SKILL.md", sums)
